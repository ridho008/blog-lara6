...
@section('content')
    <div class="row">
        ...
        @foreach ($data as $posts)
            <div class="col-md-6">
                <div class="post">
                    <a class="post-img" href="{{ route('blog.content', $posts->slug) }}"><img
                            src="{{ asset('uploads/posts/' . $posts->photo) }}" alt="{{ $posts->title }}" height="200px"></a>
                    <div class="post-body">
                        <div class="post-category">
                            <a href="{{ route('blog.category', $posts->slug) }}">{{ $posts->categori->name }}</a>
                        </div>
                        <h3 class="post-title"><a href="{{ route('blog.content', $posts->slug) }}">{{ $posts->title }}</a>
                        </h3>
                        <ul class="post-meta">
