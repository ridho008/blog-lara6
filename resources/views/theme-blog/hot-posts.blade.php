<?php
try {
    $hotPostsRight = App\Posts::inRandomOrder()->take(2)->get();

    $hotPostsLeft = App\Posts::inRandomOrder()->take(1)->get();
} catch (Exception $e) {
    dd($e->getMessage());
}
?>
            ...
            <div class="col-md-8 hot-post-left">
                ...
                @foreach ($hotPostsLeft as $hotPLeft)
                    <div class="post post-thumb">
                        <a class="post-img" href="{{ route('blog.content', $hotPLeft->slug) }}">
                            <img src="{{ asset('uploads/posts/' . $hotPLeft->photo) }}" alt=""></a>
                        <div class="post-body">
                            <div class="post-category">
                                <a
                                    href="{{ route('blog.category', $hotPLeft->slug) }}">{{ $hotPLeft->categori->name }}</a>
                            </div>
                            <h3 class="post-title title-lg"><a
                                    href="{{ route('blog.content', $hotPLeft->slug) }}">{{ $hotPLeft->title }}</a></h3>
                            <ul class="post-meta">
